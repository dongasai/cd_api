...
class AdminTablesSeeder extends Seeder
{
    ...
    public function run()
    {
        // base tables - 只插入数据，不执行 truncate
        // 如果需要重置数据，请手动清空表或使用 migrate:fresh
        if (Models\Menu::count() === 0) {
            Models\Menu::insert(
                [
                    [
                        'id' => 50,
        ...
        if (Models\ExtensionHistory::count() === 0) {
        ...
        // pivot tables - 只在空表时插入
        if (DB::table('admin_permission_menu')->count() === 0) {
