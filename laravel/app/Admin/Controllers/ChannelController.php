...
class ChannelController extends AdminController
{
    ...
    protected function detail($id)
    {
        ...
        return Show::make($channel, function (Show $show) {
            ...
            $show->field('provider')->using(admin_trans('channel.options.provider'))->width(3);
            ...
            $show->field('inherit_mode')->using(admin_trans('channel.options.inherit_mode'))->width(3);
        ...
        return Form::make(Channel::class, function (Form $form) {
            ...
            $form->tab(admin_trans_label('basic_info'), function (Form $form) {
                ...
                $form->select('provider')
                    ->options(admin_trans('channel.options.provider'))
                    ->required()
            ...
            $form->tab(admin_trans_label('status_settings'), function (Form $form) {
                $form->select('status')->options(admin_trans('channel.options.status'))
                    ->default('active')->required();

                $form->select('status2', admin_trans_field('health_status'))
                    ->options(admin_trans('channel.options.status2'))
            ...
            $form->tab(admin_trans_label('advanced_config'), function (Form $form) {
                ...
                $form->select('inherit_mode')->options(admin_trans('channel.options.inherit_mode'))
                    ->default('merge');
            });
